...
namespace GuideSetsDeterminant.Creator
{
    public sealed class GuideSetCreator
    {
        ...
        private List<string> CalculateEmptyCurrent( string token )
        {
            ...
            for ( var i = 0; i < Sentences.Count; ++i )
            {
                if ( !_stackOfEmpties.Contains( i ) && Sentences[ i ].Tokens.Contains( token ) )
                {
                    var currentIndex = 0;
                    while ( currentIndex < Sentences[ i ].Tokens.Count )
                    {
                        ProcessToken(generatedSet, token, i, currentIndex);
        ...
        private void ProcessToken(List<string> generatedSet, string token, int i, int currentIndex)
        {
